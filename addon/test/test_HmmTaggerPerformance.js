...
let hmmTagger = new freeling.HmmTagger(cnfPath + cnfLang + cnfTagger, true, 'FORCE_TAGGER');
      ...
      .then((wordLists) => {
        ...
        Promise.all([splitter.split(lw1658tokens), splitter.split(lw8368tokens)])
          .then((sentenceLists) => {
            ...
            Promise.all([morfo.analyze(ls1658tokens), morfo.analyze(ls8368tokens)])
              .then((results) => {
                const data1658tokens = results[0];
                const data8368tokens = results[1];

                const analyzePerformanceStart = new jm({ isPrint, isKb });
                hmmTagger.analyze(data1658tokens)
                  .then((result) => {
                    console.log(`=== analyze 1658 performance ===`);
                    const analyzePerformance = analyzePerformanceStart.stop();
                  })
                hmmTagger.analyze(data8368tokens)
                  .then((result) => {
