...
class AnalyzedSentence extends Component {
  ...
  ChangeSize=()=>{
    const newHeight=this.state.height==="300px"?
      "auto":"300px";
    const newIconTransform=this.state.iconTransform==="rotate(0deg)"?
      "rotate(180deg)":"rotate(0deg)";
    this.setState({
      height:newHeight,
      iconTransform:newIconTransform
    });
  };
  ...
  render(){
    return (
      <div className="analyzed-sentence-wrapper">
        <div className="analyzed-sentence" style={{height:this.state.height}}
          ...
          <header className="sentence-header">{this.props.number}</header>
            {
              this.props.sentence.map((word,index)=>{ //word is an object && option can be morf or pos-tag
                return <AnalyzedWord key={"aword"+index+1}
                                     word={word}
